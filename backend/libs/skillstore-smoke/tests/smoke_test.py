...
def test_document_upload(browser):
    ...
    try:
        ...
        # Use exact role match to avoid hitting the "+ Create Your First Context" button
        # in MyContextsList, which also contains the text "Create" and is always in the DOM.
        # The ContextBuilder input placeholder is: e.g. "Admissions 2024" or "Brand Guide"
        try:
        ...
        # Step 4b: Confirm the file entry appeared in the list (onChange fired, state updated)
        try:
        ...
        # Use XPath OR — "text=A, text=B" is not valid Playwright selector syntax
        try:
            p.wait_for_selector(
